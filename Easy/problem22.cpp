        ...
        for(int i = 0;i<n;i++){
            for(int j = 0;j < m;j++){
                ...
                if(matrix[i][j] == 1){
                    ...
                    if(j > 0 && matrix[i][j-1] == 0 )//1 pos to left
                       zeros++;
                    if (i > 0 && j > 0 && matrix[i-1][j-1] == 0)//up and left
                      zeros++;
                    if( j < m - 1 && matrix[i][j+1] == 0)//1 pos to right
                        zeros++;
                    if( i > 0 && j < m-1 && matrix[i-1][j+1] == 0)//up and right
                        zeros++;
                    if(i < n - 1 && matrix[i+1][j] == 0 )//1 pos down
                        zeros++;
                    if(i < n-1 && j  > 0 && matrix[i+1][j-1] == 0)
                        zeros++;
                    if( i < n-1 && j < m-1 && matrix[i+1][j+1] == 0)
                        zeros++;
                    if( i > 0 && matrix[i-1][j] == 0)//1 pos up
                        zeros++;
